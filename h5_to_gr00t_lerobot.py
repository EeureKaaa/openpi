...
def parse_args():
    parser = argparse.ArgumentParser(description='Convert h5 dataset to GR00T-compatible LeRobot format')
    ...
    parser.add_argument('--output_dir', type=str, default='/home/wangxianhao/data/project/reasoning/openpi/gr00t_dataset', help='Output directory for GR00T LeRobot dataset')
    parser.add_argument('--dataset_name', type=str, default='primitive_dataset', help='Name of the dataset')
    ...
    parser.add_argument('--state_dim', type=int, default=8, help='Dimension of the state vector (qpos)')
    parser.add_argument('--action_dim', type=int, default=7, help='Dimension of the action vector')
    parser.add_argument('--generate_videos', action='store_true', help='Generate videos from frames')
...
def process_episode(h5_dataset, episode_idx, output_dir, args, env_name=None, task_instruction=None):
    ...
    if num_actions < 8:
        print(f"Warning: Episode {episode_idx} has only {num_actions} actions (less than 8), skipping")
        return None
    ...
    for i in range(num_actions):
        ...
        next_dones.append(bool(traj_group['terminated'][i] or traj_group['truncated'][i]))
    ...
    base_video_success = create_video_from_frames(base_frames, base_video_path, args.fps)
    wrist_video_success = create_video_from_frames(wrist_frames, wrist_video_path, args.fps)
    left_video_success = create_video_from_frames(left_frames, left_video_path, args.fps)
...
def main():
    ...
    for env_name in env_dirs:
        ...
        h5_files = glob.glob(os.path.join(env_dir, '**', '*.h5'), recursive=True)
        ...
        for h5_file in h5_files:
            ...
            end_time = time.time()
            print(f"Finished processing {h5_file} in {end_time - start_time:.2f} seconds")
